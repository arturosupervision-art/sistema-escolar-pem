...
# Este componente registra el Service Worker para permitir instalación y Push
PWA_PUSH_SCRIPT = """
<script>
...
# Renderizar botón de activación Push en PWA
...
def modulo_carga_datos(key_prefix=""):
        ...
        if archivo and st.button("🚀 Procesar e Importar Alumnos", key=f"{key_prefix}_btn_proc_al"):
            ...
            for _, f in df.iterrows():
                try:
                    ...
                    cursor.execute("INSERT INTO alumnos VALUES (?, ?, ?, ?, ?, ?)", (str(f['matricula']).strip(), str(f['nombre']).strip(), int(f['semestre']), str(f['grupo']).strip().upper(), ct, wa))
                    ex += 1
                except: err += 1
            conn.commit()
            conn.close()
    ...
    # 1. REPORTE ACADÉMICO (INDIVIDUAL Y MASIVO)
    if opcion == "Nuevo Reporte Académico":
        ...
        with tab_mas:
            ...
            # SECCIÓN FIJA DE BOTONES DE WHATSAPP
...
if st.session_state.rol in ["Subdirector", "Coordinación"]:
    ...
    if st.session_state.rol == "Subdirector":
        ...
        with pestanas[4]: st.write("Módulo de alta de usuarios activos.")
    else:
