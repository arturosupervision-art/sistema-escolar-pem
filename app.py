...
def generar_link_whatsapp(telefono, nombre_alumno, tipo_evento, detalle):
    ...
    texto = (
        f"🏛️ *Preparatoria Estado de México*\n"
        f"Estimado tutor, le notificamos un registro de tipo *{tipo_evento}* para el alumno(a): *{nombre_alumno}*.\n\n"
        f"📌 *Detalle/Motivo:* {detalle}\n"
        f"📅 *Fecha:* {datetime.now().strftime('%d/%m/%Y %H:%M')}\n\n"
        f"Para aclaraciones, favor de acudir a la Subdirección Académica."
    )
    texto_enc = urllib.parse.quote(texto)
    return f"https://wa.me/{tel_limpio}?text={texto_enc}"

def registrar_evidencia_notificacion(reporte_id, metodo):
    """Guarda en la base de datos la fecha, hora exacta y medio como evidencia oficial legal/administrativa"""
    conn = sqlite3.connect("sistema_escolar.db")
    cursor = conn.cursor()
    estampa = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    cursor.execute("UPDATE reportes SET metodo_notificacion = ? WHERE id = ?", (f"{metodo} (Certificado el {estampa})", reporte_id))
    conn.commit()
    conn.close()
...
def inicializar_bd():
    ...
    try: cursor.execute("ALTER TABLE alumnos ADD COLUMN correo_tutor TEXT")
    except: pass
    try: cursor.execute("ALTER TABLE alumnos ADD COLUMN whatsapp_tutor TEXT")
    except: pass
    ...
    with st.form("Login"):
        ...
        if st.form_submit_button("Ingresar al Sistema"):
            ...
            if res:
                ...
                cookie_manager.set("pem_usuario", usuario, key="set_usr", expires_at=datetime.now() + pd.Timedelta(days=7))
                cookie_manager.set("pem_rol", rol_db, key="set_rol", expires_at=datetime.now() + pd.Timedelta(days=7))
                st.rerun()
            else: 
...
def generar_excel_muestra_rep_academicos():
    df = pd.DataFrame({"matricula": ["2026001"], "semestre": [3], "fecha": [datetime.now().strftime("%Y-%m-%d")], "motivo": ["I Inasistencia"], "metodo_notificacion": ["Mensaje de WhatsApp"]})
    out = io.BytesIO()
    with pd.ExcelWriter(out, engine='openpyxl') as w: df.to_excel(w, index=False)
    return out.getvalue()
...
def modulo_carga_datos(key_prefix=""):
    ...
    if opcion == "Registrar Nuevo Alumno":
        ...
        with st.form(f"{key_prefix}_form_alumno"):
            ...
            correo_t = st.text_input("Correo del Tutor/Padre")
    ...
    if opcion == "Nuevo Reporte Académico":
        ...
        with tab_ind:
            ...
            metodo_ac = st.selectbox("¿Medio de Notificación Empleado?", OPCIONES_NOTIFICACION, key=f"{key_prefix}_met_ac")
            ...
            st.info(f"📧 **Correo:** {correo_tutor or 'Sin correo'} | 📱 **WhatsApp:** {wa_tutor or 'Sin teléfono'}")
            
            if st.button("Registrar Reporte Académico", key=f"{key_prefix}_btn_ac"):
                if motivo_final.strip():
                    conn = sqlite3.connect("sistema_escolar.db")
                    cursor = conn.cursor()
                    cursor.execute("INSERT INTO reportes VALUES (NULL, ?, ?, ?, ?, ?, 'Académico')", (mat_limpia, sem_rep, datetime.now().strftime("%Y-%m-%d"), motivo_final.strip(), metodo_ac))
                    reporte_id = cursor.lastrowid
                    conn.commit()
                    conn.close()
                    
                    ok_c, _ = enviar_notificacion_correo(correo_tutor, nom_alumno, mat_limpia, "Reporte Académico", f"Motivo: {motivo_final}")
                    
                    st.success("✅ Reporte Académico guardado en el expediente del alumno.")
                    
                    st.markdown("---")
                    st.markdown("#### 📜 Registro de Evidencia / Acuse de Notificación Escolar")
                    st.info(f"Medio Seleccionado: **{metodo_ac}**")
                    
                    if metodo_ac == "Mensaje de WhatsApp" and wa_tutor:
                        link_wa = generar_link_whatsapp(wa_tutor, nom_alumno, "Académico", motivo_final)
                        st.markdown(f'<a href="{link_wa}" target="_blank" style="background-color:#25D366; color:white; padding:10px 18px; text-decoration:none; border-radius:6px; font-weight:bold; display:inline-block; margin-bottom:10px;">💬 Abrir Mensaje en WhatsApp</a>', unsafe_allow_html=True)
                    
                    if st.button("📌 Certificar y Registrar Evidencia de Emisión (Bitácora Escolar)", key=f"{key_prefix}_btn_evid_ac_gen"):
                        registrar_evidencia_notificacion(reporte_id, metodo_ac)
                        st.success("🏛️ ¡Evidencia con estampilla de fecha y hora guardada en el sistema!")
        ...
        with tab_mas:
            ...
            if file_ac and st.button("🚀 Procesar Carga Masiva", key=f"{key_prefix}_btn_proc_mas"):
                ...
                for _, f in df_rep.iterrows():
                    ...
                    metodo_item = str(f['metodo_notificacion']).strip() if 'metodo_notificacion' in df_rep.columns and pd.notnull(f['metodo_notificacion']) else "Multicanal"
                    
                    cursor.execute("INSERT INTO reportes VALUES (NULL, ?, ?, ?, ?, ?, 'Académico')", (mat_item, sem_item, fecha_item, motivo_item, metodo_item))
                    ...
                    if al_data:
                        ...
                        link = generar_link_whatsapp(wa_t, nom_a, "Académico", motivo_item) if wa_t else None
                        temp_links.append({"ID": rep_id, "Matrícula": mat_item, "Alumno": nom_a, "Método": metodo_item, "Motivo": motivo_item, "Link": link})
                
                conn.commit()
                conn.close()
                st.session_state.links_masivos_wa = temp_links
                st.success(f"🎉 ¡Proceso masivo completado!\n- 📋 {c_ok} Reportes procesados.\n- 📧 {c_correos} Correos automáticos enviados.")

            if st.session_state.links_masivos_wa:
                st.markdown("---")
                st.markdown("### 💬 Bitácora Activa para Confirmación y Registro de Evidencias")
                st.info("Utiliza esta lista para emitir/certificar las evidencias de emisión de cada reporte masivo:")
                
                for idx, item in enumerate(st.session_state.links_masivos_wa):
                    col_a, col_b, col_c = st.columns([3, 1.2, 1.2])
                    with col_a: 
                        st.write(f"👤 **{item['Alumno']}** ({item['Matrícula']}) - *{item['Motivo']}* | Método: **{item['Método']}**")
                    with col_b: 
                        if item['Link']:
                            st.markdown(f'<a href="{item["Link"]}" target="_blank" style="background-color:#25D366; color:white; padding:6px 14px; text-decoration:none; border-radius:5px; font-size:12px; font-weight:bold; display:inline-block;">💬 Abrir WA</a>', unsafe_allow_html=True)
                        else:
                            st.caption("Sin WhatsApp")
                    with col_c:
                        if st.button("📌 Certificar Evidencia", key=f"btn_evid_mas_{item['ID']}_{idx}"):
                            registrar_evidencia_notificacion(item['ID'], item['Método'])
                            st.success("✓ Registrado")

                if st.button("🧹 Limpiar Lista", key=f"{key_prefix}_clear_wa"):
                    st.session_state.links_masivos_wa = []
                    st.rerun()
    ...
    # 2. REPORTE DISCIPLINARIO
        ...
        metodo_notif = st.selectbox("¿Medio de Notificación Empleado?", OPCIONES_NOTIFICACION, index=0)
        fecha_rep = datetime.now().strftime("%Y-%m-%d")
        ...
        if st.button("Registrar Reporte Disciplinario"):
            if motivo_disc.strip():
                conn = sqlite3.connect("sistema_escolar.db")
                cursor = conn.cursor()
                cursor.execute("INSERT INTO reportes VALUES (NULL, ?, ?, ?, ?, ?, 'Disciplinario')", (mat_limpia, sem_rep, fecha_rep, motivo_disc.strip(), metodo_notif))
                reporte_id = cursor.lastrowid
                conn.commit()
                conn.close()
                
                ok_c, _ = enviar_notificacion_correo(correo_tutor, nom_alumno, mat_limpia, "Reporte Disciplinario", f"Detalle: {motivo_disc}")
                
                st.success("✅ Reporte Disciplinario Registrado.")
                
                st.markdown("---")
                st.markdown("#### 📜 Registro de Evidencia / Acuse de Notificación Escolar")
                st.info(f"Medio Seleccionado: **{metodo_notif}**")
                
                if metodo_notif == "Mensaje de WhatsApp" and wa_tutor:
                    link_wa = generar_link_whatsapp(wa_tutor, nom_alumno, "Disciplinario", motivo_disc)
                    st.markdown(f'<a href="{link_wa}" target="_blank" style="background-color:#25D366; color:white; padding:10px 18px; text-decoration:none; border-radius:6px; font-weight:bold; display:inline-block; margin-bottom:10px;">💬 Abrir Mensaje en WhatsApp</a>', unsafe_allow_html=True)
                
                if st.button("📌 Certificar y Registrar Evidencia de Emisión (Bitácora Escolar)", key=f"{key_prefix}_btn_evid_disc_gen"):
                    registrar_evidencia_notificacion(reporte_id, metodo_notif)
                    st.success("🏛️ ¡Evidencia de notificación guardada con estampilla de fecha y hora!")
    ...
    # 3. NUEVA CALIFICACIÓN
        ...
        metodo_cal = st.selectbox("Medio de Notificación / Publicación:", OPCIONES_NOTIFICACION)
        ...
        if st.button("Guardar Calificación"):
            ...
            # Registrar además un acuse en la bitácora general de reportes/notificaciones
            estampa_now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            cursor.execute("INSERT INTO reportes VALUES (NULL, ?, ?, ?, ?, ?, 'Aviso Calificación')", (mat_limpia, sem_calif, datetime.now().strftime("%Y-%m-%d"), f"Registro/Modificación de calificación: {materia_selec} ({tipo_parcial}): {calif_nota}", f"{metodo_cal} (Certificado el {estampa_now})"))
            
            conn.commit()
            conn.close()
            
            enviar_notificacion_correo(correo_tutor, nom_alumno, mat_limpia, "Calificación", f"Materia: {materia_selec}<br>Nota: {calif_nota}")
            st.success(f"✅ Calificación guardada y acuse certificado registrado bajo medio: {metodo_cal}.")
    ...
    # 4. AYUDA ACADÉMICA
        ...
        metodo_ayuda = st.selectbox("Medio de Notificación al Tutor:", OPCIONES_NOTIFICACION)
        
        if st.button("Guardar Ayuda y Certificar Evidencia"):
            conn = sqlite3.connect("sistema_escolar.db")
            cursor = conn.cursor()
            cursor.execute("INSERT INTO ayuda VALUES (NULL, ?, ?, ?, ?)", (mat_limpia, sem_a, tipo_a, obs_a))
            
            # Registrar acuse en la bitácora de notificaciones
            estampa_now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            cursor.execute("INSERT INTO reportes VALUES (NULL, ?, ?, ?, ?, ?, 'Apoyo/Tutoría')", (mat_limpia, sem_a, datetime.now().strftime("%Y-%m-%d"), f"{tipo_a}: {obs_a}", f"{metodo_ayuda} (Certificado el {estampa_now})"))
            
            conn.commit()
            conn.close()
            st.success("✅ Registro de apoyo y evidencia de notificación guardados correctamente.")

# ----------------- VISTA DE TUTORÍAS Y BITÁCORA DE EVIDENCIAS -----------------
def modulo_tutorias():
    st.title("🎯 Módulo de Tutorías - Concentrado Diario y Períodos")
    st.info("Visualización general desglosada por Día, Semestre y Grupo con acuses y evidencias oficiales.")
    
    col_f1, col_f2, col_f3 = st.columns(3)
    with col_f1: fecha_sel = st.date_input("Filtrar por Fecha/Día:", datetime.now())
    ...
    if not df_tutoria.empty:
        df_tutoria.columns = ["ID", "Fecha", "Matrícula", "Nombre Alumno", "Semestre", "Grupo", "Tipo Evento", "Motivo/Detalle", "Método Notificación / Evidencia Certificada", "WhatsApp Tutor"]
        st.dataframe(df_tutoria, use_container_width=True, hide_index=True)
        
        out_tut = io.BytesIO()
        with pd.ExcelWriter(out_tut, engine='openpyxl') as writer:
            df_tutoria.to_excel(writer, index=False, sheet_name='Concentrado_Tutorias')
            
        st.download_button(
            label="📥 Descargar Base de Datos Filtrada con Evidencias (Excel)",
            data=out_tut.getvalue(),
            file_name=f"Concentrado_Evidencias_{fecha_str}_Sem_{sem_sel}_Gpo_{gpo_sel or 'TODOS'}.xlsx",
...
if st.session_state.rol in ["Subdirector", "Coordinación"]:
    p_tabs = ["🔍 Buscador Central", "📋 Administración de Alumnos", "📝 Captura de Datos", "🎯 Tutorías", "⚙️ Usuarios"] if st.session_state.rol == "Subdirector" else ["📝 Captura de Datos", "🎯 Tutorías", "🔍 Expedientes"]
