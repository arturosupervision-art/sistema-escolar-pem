...
# ----------------- ADMINISTRADOR DE COOKIES (SESIÓN PERSISTENTE) -----------------
def get_cookie_manager():
    return stx.CookieManager(key="cookie_manager_pem")

cookie_manager = get_cookie_manager()

# ----------------- SCRIPT PWA & NOTIFICACIONES PUSH EN CELULAR -----------------
PWA_PUSH_SCRIPT = """
<script>
...
def generar_link_whatsapp(telefono, nombre_alumno, tipo_reporte, motivo):
    if not telefono:
        return None
    ...
    texto = (
        ...
        f"Estimado tutor, le notificamos un *Reporte {tipo_reporte}* registrado para el alumno(a): *{nombre_alumno}*.\n\n"
        f"📌 *Motivo:* {motivo}\n"
...
# ----------------- AUTENTICACIÓN CON PERSISTENCIA VÍA COOKIES -----------------
if "autenticado" not in st.session_state:
    st.session_state.autenticado = False
    st.session_state.rol = None
    st.session_state.usuario = None

# Verificar si hay una cookie de sesión guardada previamente
...
# --- SIDEBAR & ACTIVACIÓN PUSH ---
if os.path.exists(LOGO_LOCAL): st.sidebar.image(LOGO_LOCAL, width=150)
    ...
    if materias:
        ...
        <div id="boleta-imprimir" style="padding: 20px; font-family: Arial, sans-serif; color: #000; width: 100%; max-width: 850px; margin: 0 auto; background: #fff;">
            <table style="width: 100%; border-bottom: 2px double #000; padding-bottom: 8px; margin-bottom: 15px;">
                <tr>
                    <td style="width: 20%; text-align: left;"><img src="{logo_embed_html}" style="width: 80px; height: auto;"></td>
        ...
        <div style="display:none;">{html_completo_pdf}</div>
        ...
        function descargarPDFHorizontal() {{
            ...
            const opciones = {{ margin: [0.5, 0.5, 0.5, 0.5], filename: 'Boleta_{matricula}_Semestre_{semestre_selec}.pdf', image: {{ type: 'jpeg', quality: 0.98 }}, html2canvas: {{ scale: 2 }}, jsPDF: {{ unit: 'in', format: 'letter', orientation: 'landscape' }} }};
            html2pdf().set(opciones).from(elemento).save();
        }}
        </script>""", height=65)
    else: st.warning("No se encontraron calificaciones registradas para este semestre.")
        ...
        for sem in semestres_disponibles:
            with st.expander(f"➔ Ver Historial Completo del {sem}° Semestre", expanded=True):
                ...
                with col1:
                    ...
                    else: st.caption("*Sin asignaturas*")
                ...
                with col2:
                    ...
                    else: st.success("✓ Conducta Excelente")
                        
                with col3:
                    st.markdown("**📍 Apoyos y Tutorías**")
                    a_sem = [x for x in ayudas if x[0] == sem]
                    if a_sem:
                        for a in a_sem: st.warning(f"🌟 {a[1]}\n{a[2]}")
                    else: st.caption("*Sin requerimientos*")
        conn.close()
...
def generar_excel_muestra_rep_academicos():
    df = pd.DataFrame({"matricula": ["2026001"], "semestre": [3], "fecha": [datetime.now().strftime("%Y-%m-%d")], "motivo": ["I Inasistencia"], "metodo_notificacion": ["Multicanal"]})
    out = io.BytesIO()
    with pd.ExcelWriter(out, engine='openpyxl') as w: df.to_excel(w, index=False)
    return out.getvalue()
...
def modulo_carga_datos(key_prefix=""):
            ...
            if st.form_submit_button("Dar de Alta Alumno"):
                if mat.strip() and nombre_al.strip():
                    ...
                    try:
                        ...
                        conn.commit()
                        st.success(f"¡Alumno {nombre_al} registrado con éxito y usuario de acceso creado (Usuario: {mat.strip()} | Pass: {mat.strip()})!")
                        st.rerun()
    ...
    if opcion == "Nuevo Reporte Académico":
        ...
        with tab_ind:
            ...
            if st.button("Registrar Reporte y Enviar Notificaciones", key=f"{key_prefix}_btn_ac"):
                if motivo_final.strip():
                    conn = sqlite3.connect("sistema_escolar.db")
                    cursor = conn.cursor()
                    cursor.execute("INSERT INTO reportes VALUES (NULL, ?, ?, ?, ?, ?, 'Académico')", (mat_limpia, sem_rep, datetime.now().strftime("%Y-%m-%d"), motivo_final.strip(), "Multicanal"))
                    conn.commit()
                    conn.close()
                    
                    ok_c, msg_c = enviar_notificacion_correo(correo_tutor, nom_alumno, mat_limpia, "Reporte Académico", f"Motivo: {motivo_final}")
                    
                    st.success("✅ Reporte Académico registrado en la base de datos.")
                    if ok_c: st.success("📧 Correo electrónico enviado al tutor.")
                    else: st.warning(f"⚠️ Correo: {msg_c}")

                    if wa_tutor:
                        link_wa = generar_link_whatsapp(wa_tutor, nom_alumno, "Académico", motivo_final)
                        st.markdown(f'<a href="{link_wa}" target="_blank" style="background-color:#25D366; color:white; padding:12px 20px; text-decoration:none; border-radius:6px; font-weight:bold; display:inline-block; margin-top:10px;">💬 Enviar Notificación por WhatsApp Business</a>', unsafe_allow_html=True)
        ...
        with tab_mas:
            ...
            if file_ac and st.button("🚀 Procesar Carga Masiva y Generar WhatsApps", key=f"{key_prefix}_btn_proc_mas"):
                ...
                for _, f in df_rep.iterrows():
                    ...
                    cursor.execute("INSERT INTO reportes VALUES (NULL, ?, ?, ?, ?, 'Multicanal', 'Académico')", (mat_item, sem_item, fecha_item, motivo_item))
                    c_ok += 1
                    ...
                    if al_data:
                        ...
                        if wa_t:
                            link = generar_link_whatsapp(wa_t, nom_a, "Académico", motivo_item)
                            temp_links.append({"Matrícula": mat_item, "Alumno": nom_a, "WhatsApp": wa_t, "Motivo": motivo_item, "Link": link})
            ...
            if st.session_state.links_masivos_wa:
                st.markdown("---")
                st.markdown("### 💬 Lista Activa de Envíos Opcionales por WhatsApp")
                st.info("Haz clic en el botón para abrir WhatsApp Web/App y enviar mensaje individual:")
                
                for idx, item in enumerate(st.session_state.links_masivos_wa):
                    col_a, col_b = st.columns([3, 1])
                    with col_a: 
                        st.write(f"👤 **{item['Alumno']}** ({item['Matrícula']}) - *{item['Motivo']}* | Tel: {item['WhatsApp']}")
                    with col_b: 
                        st.markdown(f'<a href="{item["Link"]}" target="_blank" style="background-color:#25D366; color:white; padding:6px 14px; text-decoration:none; border-radius:5px; font-size:12px; font-weight:bold; display:inline-block;">💬 Enviar WhatsApp</a>', unsafe_allow_html=True)

                if st.button("🧹 Limpiar Lista de WhatsApps", key=f"{key_prefix}_clear_wa"):
                    st.session_state.links_masivos_wa = []
                    st.rerun()
    ...
    # 2. REPORTE DISCIPLINARIO
        ...
        if st.button("Registrar Reporte Disciplinario"):
            if motivo_disc.strip():
                conn = sqlite3.connect("sistema_escolar.db")
                cursor = conn.cursor()
                cursor.execute("INSERT INTO reportes VALUES (NULL, ?, ?, ?, ?, ?, 'Disciplinario')", (mat_limpia, sem_rep, fecha_rep, motivo_disc.strip(), "Multicanal"))
                conn.commit()
                conn.close()
                
                ok_c, msg_c = enviar_notificacion_correo(correo_tutor, nom_alumno, mat_limpia, "Reporte Disciplinario", f"Detalle: {motivo_disc}")
                
                st.success("✅ Reporte Disciplinario Registrado.")
                if ok_c: st.success("📧 Notificación por correo enviada.")
                
                if wa_tutor:
                    link_wa = generar_link_whatsapp(wa_tutor, nom_alumno, "Disciplinario", motivo_disc)
                    st.markdown(f'<a href="{link_wa}" target="_blank" style="background-color:#25D366; color:white; padding:12px 20px; text-decoration:none; border-radius:6px; font-weight:bold; display:inline-block; margin-top:10px;">💬 Enviar por WhatsApp Business</a>', unsafe_allow_html=True)
...
def modulo_tutorias():
    ...
    st.info("Visualización general desglosada por Día, Semestre y Grupo con descarga de datos.")
    ...
    if not df_tutoria.empty:
        df_tutoria.columns = ["ID", "Fecha", "Matrícula", "Nombre Alumno", "Semestre", "Grupo", "Tipo Reporte", "Motivo/Detalle", "Método Notificación", "WhatsApp Tutor"]
        ...
        st.download_button(
            label="📥 Descargar Base de Datos Filtrada (Excel)",
            data=out_tut.getvalue(),
            file_name=f"Concentrado_Tutorias_{fecha_str}_Sem_{sem_sel}_Gpo_{gpo_sel or 'TODOS'}.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
        )
    else: st.warning("No hay reportes registrados para la fecha y filtros seleccionados.")
    ...
    if st.session_state.rol == "Subdirector":
        ...
        with pestanas[1]:
            ...
            if alumnos_lista:
                ...
                with col_m1: alumnos_seleccionados = st.multiselect("Selecciona los alumnos:", df_admin["Matrícula"].tolist())
                with col_m2: accion_masiva = st.selectbox("¿Qué acción masiva aplicar?", ["---", "Cambiar de Semestre Masivo", "Cambiar de Grupo Masivo", "Eliminar Alumnos Seleccionados"])
                with col_m3:
                ...
                with col_ind2:
                    ...
                    if st.button("❌ Confirmar Eliminación Individual"):
                        ...
                        st.rerun()
            else: st.info("No hay alumnos registrados actualmente.")
