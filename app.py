...
def generar_excel_muestra_rep_academicos():
    df = pd.DataFrame({"matricula": ["2026001"], "semestre": [3], "fecha": [datetime.now().strftime("%Y-%m-%d")], "motivo": ["I Inasistencia"]})
    out = io.BytesIO()
    with pd.ExcelWriter(out, engine='openpyxl') as w: df.to_excel(w, index=False)
    return out.getvalue()
...
def modulo_carga_datos(key_prefix=""):
    ...
    # ESTAMPILLA UNIFICADA DE FECHA Y HORA (DE REGISTRO Y ENVÍO)
    estampa_fecha_hora = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

    # 1. REPORTE ACADÉMICO (SIN SELECTOR DE MEDIO)
        ...
        with tab_ind:
            ...
            if st.button("Registrar Reporte Académico", key=f"{key_prefix}_btn_ac"):
                if motivo_final.strip():
                    ...
                    conn = sqlite3.connect("sistema_escolar.db")
                    cursor = conn.cursor()
                    cursor.execute("INSERT INTO reportes VALUES (NULL, ?, ?, ?, ?, ?, 'Académico')", (mat_limpia, sem_rep, datetime.now().strftime("%Y-%m-%d"), motivo_final.strip(), evidencia_auto))
                    conn.commit()
                    conn.close()
                    
                    enviar_notificacion_correo(correo_tutor, nom_alumno, mat_limpia, "Reporte Académico", f"Motivo: {motivo_final}")
                    
                    st.success(f"✅ ¡Reporte guardado! Registrado con estampilla de tiempo: {estampa_fecha_hora}")
            ...
            if file_ac and st.button("🚀 Procesar Carga Masiva", key=f"{key_prefix}_btn_proc_mas"):
                ...
                for _, f in df_rep.iterrows():
                    ...
                    if al_data:
                        ...
                        temp_links.append({"Matrícula": mat_item, "Alumno": nom_a, "Motivo": motivo_item, "Link": link})
                
                conn.commit()
                conn.close()
                st.session_state.links_masivos_wa = temp_links
                st.success(f"🎉 ¡Proceso masivo completado!\n- 📋 {c_ok} Reportes e evidencias registrados automáticamente en bitácora.\n- 📧 {c_correos} Correos enviados.")

            if st.session_state.links_masivos_wa:
                st.markdown("---")
                st.markdown("### 💬 Enlaces WhatsApp Generados")
                for item in st.session_state.links_masivos_wa:
                    col_a, col_b = st.columns([3, 1])
                    with col_a: st.write(f"👤 **{item['Alumno']}** ({item['Matrícula']}) - *{item['Motivo']}*")
                    with col_b: 
                        if item['Link']:
                            st.markdown(f'<a href="{item["Link"]}" target="_blank" style="background-color:#25D366; color:white; padding:5px 10px; text-decoration:none; border-radius:4px; font-size:12px; font-weight:bold;">💬 Abrir WA</a>', unsafe_allow_html=True)

    # 2. REPORTE DISCIPLINARIO (ÚNICO LUGAR CON SELECTOR DE MEDIO DE NOTIFICACIÓN)
        ...
        if st.button("Registrar Reporte Disciplinario"):
            if motivo_disc.strip():
                ...
                if metodo_notif == "Mensaje de WhatsApp" and wa_tutor:
                    ...
                    st.markdown(f'<a href="{link_wa}" target="_blank" style="background-color:#25D366; color:white; padding:10px 18px; text-decoration:none; border-radius:6px; font-weight:bold; display:inline-block; margin-top:8px;">💬 Abrir Mensaje Directo en WhatsApp</a>', unsafe_allow_html=True)

    # 3. NUEVA CALIFICACIÓN (SIN SELECTOR DE MEDIO)
        ...
        if st.button("Guardar Calificación"):
            ...
            # Registro Automático en la Bitácora con la estampilla
            evidencia_auto = f"Registro Sistema [{estampa_fecha_hora}]"
            cursor.execute("INSERT INTO reportes VALUES (NULL, ?, ?, ?, ?, ?, 'Aviso Calificación')", (mat_limpia, sem_calif, datetime.now().strftime("%Y-%m-%d"), f"{materia_selec} ({tipo_parcial}): {calif_nota}", evidencia_auto))
    ...
    # 4. AYUDA ACADÉMICA (SIN SELECTOR DE MEDIO)
        ...
        if st.button("Guardar Ayuda y Certificar"):
            ...
            # Registro Automático en la Bitácora con la estampilla
            evidencia_auto = f"Registro Sistema [{estampa_fecha_hora}]"
            cursor.execute("INSERT INTO reportes VALUES (NULL, ?, ?, ?, ?, ?, 'Apoyo/Tutoría')", (mat_limpia, sem_a, datetime.now().strftime("%Y-%m-%d"), f"{tipo_a}: {obs_a}", evidencia_auto))
