...
LOGO_PATH = os.path.join(BASE_DIR, "logo 172.png")  # Nombre exacto de tu archivo en GitHub
...
def mostrar_boleta(matricula, nombre, grupo, semestre_selec):
    ...
    if materias:
        ...
        # Generar la etiqueta de imagen solo si existe la representación base64
        ...
        <!-- Contenedor posicionado fuera de pantalla pero visible para el renderizador gráfico -->
        ...
        with tab_ind:
            ...
            if st.button("Registrar Reporte y Enviar Notificaciones", key=f"{key_prefix}_btn_ac"):
                if motivo_final.strip():
                    ...
                    if ok_c: 
                        st.success("📧 Correo electrónico enviado al tutor.")
                    else: 
                        st.warning(f"⚠️ Correo: {msg_c}")
...
def modulo_tutorias():
    ...
    if not df_tutoria.empty:
        ...
        st.warning("No hay reportes registrados para la fecha y filtros seleccionados.")
